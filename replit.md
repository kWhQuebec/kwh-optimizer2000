...
-   **Multi-Scenario Acquisition Cashflow Charts**: PDF, HTML presentation, and PPTX all render 3 cumulative cashflow lines: Comptant (green, solid), Prêt (blue, dashed — 10yr/7%/30% down), Crédit-bail 15 ans (gold, dashed — 8.5%). Server utility at `server/services/acquisitionCashflows.ts`. Uses "Options d'acquisition" (never "Options de financement").
