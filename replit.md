...
-   **Consumption-Based Quick Analysis with Dynamic Optimization**: Calculates solar system sizing from annual kWh consumption (via bill upload or manual entry). Analyzes 11 scenarios from 25% to 125% offset in 10% increments, then returns 3 optimized scenarios: **"Meilleur retour"** (shortest payback period, recommended), **"Maximum"** (100% offset reference), and either **"Meilleur LCOE"** (lowest 25-year energy cost) or **"Optimal"** (intermediate option). No Google Solar API dependency for sizing. **Defaults to bill upload mode**. Methodology constants: SOLAR_YIELD=1000 kWh/kWp, COST_PER_KW=$2250, HQ incentive $1000/kW (max 40% CAPEX, 1MW limit), Federal ITC 30%.
