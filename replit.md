...
-   **Financing Calculator**: Compares Cash, Loan, Lease, and PPA options over 25-year horizon with 0.5%/yr panel degradation on all curves. PPA defaults: 85% Year 2+ rate (15% savings), 15-year term, 10% CAPEX buyout at end of term. Loan defaults: 10yr/5.5%/20% down. Capital lease rebates (HQ solar + 50% battery) reduce financed amount rather than appearing as upfront cash.
-   **Multi-Scenario Acquisition Cashflow Charts**: PDF, HTML presentation, and PPTX all render 3 cumulative cashflow lines: Comptant (green, solid), Prêt (blue, dashed — 10yr/5.5%/20% down), Crédit-bail 15 ans (gold, dashed — 8.5%). Server utility at `server/services/acquisitionCashflows.ts`. Uses "Options d'acquisition" (never "Options de financement").
-   **Email Lead Nurturing**: Automated 6-email educational sequence (days 3, 7, 10, 14, 21, 30) sent to new prospects. Features: global ON/OFF toggle in Admin Settings (`/app/admin/settings`), per-lead controls (start/pause/stop/resume) in pipeline opportunity detail dialog's "Nurturing" tab, visual progress indicator showing sent/pending/cancelled status for each email. System setting stored in `system_settings` table. Scheduler polls every 15 minutes and respects both global and per-lead nurture status. Leads have `nurtureStatus` field (null/active/paused/stopped/completed).
