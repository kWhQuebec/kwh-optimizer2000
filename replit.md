...
The Dream Industrial REIT Master Agreement PDF (`server/pdf/masterAgreementPDF.ts`) generates a comprehensive Design-Build (CCDC 14) document — NOT a PPA. Sections: Cover, TOC, Executive Summary (3 KPIs: Sites, kW DC, Investment), Parties & Roles, Commercial Terms (Design-Build model, no PPA terms), Financial Framework ($/W pricing column, auto-splits for >15 sites), Scope of Work (Section 5: 11 Owner items + 16 Design-Builder items including ITC/LCAB + 5 General Provisions), Schedule of Values (Section 6: 7 milestones matching Dream Exhibit C — 15%/10%/15%/25%/20%/10%/5%, consolidated to 1 page, 20 pricing assumptions, 9 exclusions, payment terms), Annex A per-site schedules (degradation display uses smart formatting to avoid 50% vs 0.5% confusion; ITC labeled "estimated"; interconnection DOT note added), Supplementary Conditions (Section 8: 28 SC articles, consistently uses "construction legal hypothec" wording, regular thermography not drone), and Signatures. All pages use fixed `.page` class (279.4mm height, overflow:hidden) with Puppeteer-side overflow detection.
