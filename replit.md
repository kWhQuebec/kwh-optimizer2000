...
-   **Financing Calculator**: Compares Cash, Loan, Lease, and PPA options over 25-year horizon with 0.5%/yr panel degradation on all curves. PPA defaults: 85% Year 2+ rate (15% savings), 15-year term, 10% CAPEX buyout at end of term. Loan defaults: 15yr/5.5%/20% down. Capital lease rebates (HQ solar + 50% battery) reduce financed amount rather than appearing as upfront cash.
-   **Multi-Scenario Acquisition Cashflow Charts**: PDF, HTML presentation, and PPTX all render 3 cumulative cashflow lines: Comptant (green, solid), Prêt (blue, dashed — 15yr/5.5%/20% down), Crédit-bail 15 ans (gold, dashed — 8.5%). Server utility at `server/services/acquisitionCashflows.ts`. Uses "Options d'acquisition" (never "Options de financement").
