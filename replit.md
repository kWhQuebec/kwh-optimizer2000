...
### Optimization Frontier Chart (PDF Integration)
PDF proposals conditionally include an "Analyse d'optimisation" page (`buildOptimizationPage` in `pdfGeneratorV2.ts`) when `simulation.sensitivity.frontier` data exists. The page renders an inline SVG scatter chart (`generateFrontierSVG`) plotting net investment (x) vs 25-year NPV (y) for all swept configurations — solar (gold), storage (blue), hybrid (green). Unprofitable points (NPV<0) are faded; the optimal point (max NPV) is highlighted with a ring. A red dashed NPV=0 profitability threshold line is drawn. A summary box identifies the optimal configuration with key metrics. OSE 6.0 incentive: battery receives 0 HQ incentive (solar-only program); no spillover from solar cap to battery.
