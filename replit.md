...
- **Feb 2026**: Master Agreement PDF rewritten as Puppeteer/HTML generator (was PDFKit). All sections: Cover, TOC, Executive Summary, Parties & Roles, Commercial Terms, Financial Framework (23 sites), Annex A per-site, General Conditions, Signatures. Route updated to pass all sites and return Buffer.
- **Feb 2026**: Portfolio page performance fix — `getPortfolioSites` now selects only lightweight simulation columns (excludes cashflows, breakdown, hourlyProfile, peakWeekData, sensitivity JSONB blobs). The `/full` API response strips `roofVisualizationImageUrl` (base64 images, 6MB each), `hqConsumptionHistory`, `baselineMonthlyProfile`, and `analysisAssumptions` from site objects. Response reduced from 64MB to 196KB.
- **Feb 2026**: Roof area fallback for analysis — when no roof polygons or `roofAreaSqM` exist, analysis endpoints now fall back to `buildingSqFt` (converted to m²) before throwing an error. Applies to both quick analysis and detailed economic validation.
