...
export function DesignAgreementSection({ siteId }: DesignAgreementSectionProps) {
  ...
  useEffect(() => {
    if (!agreementLoading && !agreement && !autoGeneratedRef.current && !generateMutation.isPending) {
      ...
      <Card data-testid="card-design-agreement">
        ...
        <CollapsibleContent>
          <CardContent className="space-y-6">
            {!agreement ? (
              <div className="text-center py-8 space-y-3" data-testid="section-agreement-preparing">
                <Loader2 className="w-8 h-8 mx-auto animate-spin text-primary" />
                <p className="text-muted-foreground text-sm">
                  {t("designAgreement.preparing")}
                </p>
              </div>
            ) : (
              <>
