...
export function generateProfessionalPDF(
  ...
  // Snapshot data grid - 2 columns, 4 rows
  ...
  snapData.forEach(([label, value], idx) => {
    ...
    drawRoundedRect(sx, sy, snapColWidth, snapRowHeight, 6, COLORS.background);
  ...
  drawRoundedRect(margin, doc.y, contentWidth, 60, 6, COLORS.background);
  ...
  // Footer
  doc.fontSize(8).fillColor(COLORS.lightGray);
  doc.text(t("Document confidentiel | Généré par kWh Québec | Aperçu du projet", "Confidential document | Generated by kWh Québec | Project snapshot"), margin, pageHeight - 30, { align: "center", width: contentWidth });
  ...
  doc.fontSize(12).fillColor(COLORS.blue).font("Helvetica-Bold");
  doc.text(t("CALCUL DE L'INVESTISSEMENT NET", "NET INVESTMENT CALCULATION"), margin, doc.y);
  doc.font("Helvetica");
  doc.y += 20;

  const waterfallItems = [
    { label: t("Coût EPC avant incitatifs", "Pre-incentive EPC cost"), value: simulation.capexGross, color: COLORS.darkGray, sign: "" },
    { label: t("Incitatif HQ Solaire", "HQ Solar Incentive"), value: simulation.incentivesHQSolar, color: COLORS.green, sign: "- " },
    { label: t("Incitatif HQ Batterie", "HQ Battery Incentive"), value: simulation.incentivesHQBattery, color: COLORS.green, sign: "- " },
    { label: t("Crédit ITC fédéral (30%)", "Federal ITC (30%)"), value: simulation.incentivesFederal, color: COLORS.green, sign: "- " },
    { label: t("Bouclier fiscal (DPA)", "Tax Shield (CCA)"), value: simulation.taxShield, color: COLORS.gold, sign: "- " },
  ];

  waterfallItems.forEach((item) => {
    doc.fontSize(10).fillColor(COLORS.mediumGray);
    doc.text(item.label, margin + 10, doc.y);
    doc.fontSize(11).fillColor(item.color).font("Helvetica-Bold");
    doc.text(`${item.sign}${formatCurrency(item.value)}`, margin + contentWidth - 140, doc.y - 1, { width: 120, align: "right" });
    doc.font("Helvetica");
    doc.y += 18;
  });

  // Separator line
  doc.rect(margin + 10, doc.y, contentWidth - 20, 2).fillColor(COLORS.blue).fill();
  doc.y += 10;

  // Net investment total
  doc.fontSize(12).fillColor(COLORS.blue).font("Helvetica-Bold");
  doc.text(t("INVESTISSEMENT NET", "NET INVESTMENT"), margin + 10, doc.y);
  doc.fontSize(14);
  doc.text(formatCurrency(simulation.capexNet), margin + contentWidth - 140, doc.y - 2, { width: 120, align: "right" });
  doc.font("Helvetica");
  ...
  // Header row
  drawRoundedRect(margin, doc.y, contentWidth, assTableRowH, 0, COLORS.blue);
  ...
  // Footer
  doc.fontSize(8).fillColor(COLORS.lightGray);
  doc.text(t("Document confidentiel | Généré par kWh Québec | Hypothèses et exclusions", "Confidential document | Generated by kWh Québec | Assumptions & exclusions"), margin, pageHeight - 30, { align: "center", width: contentWidth });

  // ================= EQUIPMENT, TIMELINE & NEXT STEPS =================
  doc.addPage();
  ...
  // === EQUIPMENT SECTION ===
  doc.fontSize(14).fillColor(COLORS.blue).font("Helvetica-Bold");
  doc.text(t("ÉQUIPEMENT INDICATIF", "INDICATIVE EQUIPMENT"), margin, doc.y);
  doc.font("Helvetica");
  doc.y += 5;
  ...
  equipment.forEach((eq, idx) => {
    ...
    drawRoundedRect(ex, doc.y, eqCardWidth, eqCardHeight, 6, COLORS.background);
  ...
  const tlStepWidth = (contentWidth - (timeline.length - 1) * 5) / timeline.length;
  const tlY = doc.y;
  const tlStepH = 55;
  ...
  timeline.forEach((tl, idx) => {
    ...
    // Step box
    const stepColor = idx === 0 ? COLORS.blue : (idx === timeline.length - 1 ? COLORS.green : COLORS.background);
    const textColor = (idx === 0 || idx === timeline.length - 1) ? COLORS.white : COLORS.darkGray;
    drawRoundedRect(tx, tlY, tlStepWidth, tlStepH, 6, stepColor);
    ...
    if (tl.duration) {
      doc.fontSize(8).fillColor(idx === 0 || idx === timeline.length - 1 ? COLORS.white : COLORS.mediumGray);
  ...
  drawRoundedRect(margin, nsY, nsColWidth, nsBlockH, 6, COLORS.background);
  ...
  drawRoundedRect(nsCol2X, nsY, nsColWidth, nsBlockH, 6, COLORS.background);
  ...
  drawRoundedRect(nsCol3X, nsY, nsColWidth, nsBlockH, 6, COLORS.background);
  ...
  // Footer
  doc.fontSize(8).fillColor(COLORS.lightGray);
  doc.text(t("Document confidentiel | Généré par kWh Québec | Équipement et prochaines étapes", "Confidential document | Generated by kWh Québec | Equipment & next steps"), margin, pageHeight - 30, { align: "center", width: contentWidth });
