...
import { getEquipment, getBatteryEquipment, getEquipmentTechnicalSummary } from "@shared/brandContent";
...
export function renderSystemElements(ctx: PDFContext) {
  ...
  doc.addPage();
  drawPageHeader(ctx, t("SYSTÈME ET ÉQUIPEMENT", "SYSTEM & EQUIPMENT"));
  ...
  drawRoundedRect(doc, margin, doc.y, contentWidth, structRowH, 0, COLORS.blue);
  ...
  structRows.forEach((row, i) => {
    ...
    drawRoundedRect(doc, margin, doc.y, contentWidth, structRowH, 0, bg);
    doc.fontSize(7).fillColor(COLORS.darkGray);
  ...
  drawPageFooter(
    ctx,
    t("Document confidentiel | Généré par kWh Québec | Système et équipement", "Confidential document | Generated by kWh Québec | System & equipment")
  );
}
