...
router.get("/api/kb-racking/estimate", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
  const { estimateKBRackingCost } = await getKBRackingModule();
  const panelCount = parseInt(req.query.panels as string) || 0;
  if (panelCount <= 0) {
    throw new BadRequestError("Panel count must be greater than 0");
  }
...
router.get("/api/kb-racking/estimate-from-area", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
...
router.get("/api/kb-racking/bom/:panelCount", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
...
router.get("/api/kb-racking/specs", authMiddleware, asyncHandler(async (req: AuthRequest, res) => {
  const { KB_RACKING_SPECS } = await getKBRackingModule();
  res.json(KB_RACKING_SPECS);
}));

router.get("/api/kb-racking/portfolio-stats", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
...
router.post("/api/kb-racking/proposal-pdf/:siteId", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
  const { estimateKBRackingCost } = await getKBRackingModule();
  ...
  if (siteData.kbDesignStatus !== 'complete' || !siteData.kbPanelCount) {
    throw new BadRequestError("Site does not have KB Racking design data");
  }
...
router.get("/api/kb-racking/google-solar-comparison", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
...
router.get("/api/kb-racking/expiring-quotes", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
