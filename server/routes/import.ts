...
router.post("/api/import/prospects/ai-parse", authMiddleware, requireStaff, uploadMemory.single("file"), asyncHandler(async (req: AuthRequest, res) => {
  if (!req.file) {
  ...
  if (fileName.endsWith(".xlsx") || fileName.endsWith(".xls")) {
    throw new BadRequestError("Excel files not yet supported. Please convert your Excel file to CSV format before uploading. In Excel, use File > Save As > CSV (Comma delimited).");
  }
...
${fileContent}`
  ...
  try {
    ...
    throw new BadRequestError("Failed to parse file content. The AI could not extract structured data from the file. Please ensure the file contains prospect data in a recognizable format.");
  }
...
router.post("/api/import/prospects/batch", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
  const { prospects } = req.body;

  if (!Array.isArray(prospects) || prospects.length === 0) {
    throw new BadRequestError("No prospects provided");
  }
