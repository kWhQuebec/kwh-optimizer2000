...
router.get("/api/work-queue/sites", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
...
router.post("/api/work-queue/delegate", authMiddleware, requireStaff, asyncHandler(async (req: AuthRequest, res) => {
  const { siteIds, recipientEmail, recipientName, instructions, language } = req.body;

  if (!siteIds || !Array.isArray(siteIds) || siteIds.length === 0) {
    throw new BadRequestError("siteIds array is required");
  }
  if (!recipientEmail || typeof recipientEmail !== "string") {
    throw new BadRequestError("recipientEmail is required");
  }
...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0; background-color: #f5f5f5;">
