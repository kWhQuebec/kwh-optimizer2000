...
router.post("/api/detailed-analysis-request", leadSubmissionLimiter, upload.any(), asyncHandler(async (req, res) => {
  ...
  if (procurationAccepted !== 'true') {
    throw new BadRequestError("Procuration must be accepted");
  }
  ...
  if (!hasFiles && !hasSavedPath && !hasParsedBillData) {
    throw new BadRequestError("At least one Hydro-Québec bill file is required");
  }
...
router.get("/api/leads", authMiddleware, requireStaff, asyncHandler(async (req, res) => {
...
router.get("/api/dashboard/stats", authMiddleware, requireStaff, asyncHandler(async (req, res) => {
  const stats = await storage.getDashboardStats();
  res.json(stats);
}));

// Pipeline stats for sales dashboard
router.get("/api/dashboard/pipeline-stats", authMiddleware, requireStaff, asyncHandler(async (req, res) => {
...
router.get("/api/search", authMiddleware, requireStaff, asyncHandler(async (req, res) => {
...
router.post("/api/parse-hq-bill", billUpload.single('file'), asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    throw new BadRequestError("No image provided. Send a file upload or imageBase64 in body.");
  }
